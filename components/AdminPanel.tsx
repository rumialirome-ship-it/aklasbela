...
const StatCard: React.FC<{ title: string; value: string | number; color: string; icon: React.ReactNode }> = ({ title, value, color, icon }) => (
    <div className="bg-slate-900/60 border border-white/5 p-6 rounded-[2rem] relative overflow-hidden group hover:border-white/10 transition-all">
...
const AdminPanel: React.FC<any> = ({ 
    ...
    topUpDealerWallet, withdrawFromDealerWallet, onRefreshData 
}) => {
    ...
    const tabs = [
        { id: 'dashboard', label: 'Nexus Dashboard', icon: Icons.chartBar },
        { id: 'games', label: 'Market Control', icon: Icons.gamepad },
        { id: 'dealers', label: 'Dealer Grid', icon: Icons.userGroup },
        { id: 'users', label: 'User Registry', icon: Icons.clipboardList },
    ];
        ...
        <div className="p-4 sm:p-8 max-w-7xl mx-auto min-h-screen">
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-12 gap-6">
                <div>
                    ...
                    <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mt-2">Centralized Command & Analytics</p>
                </div>
                            ...
                            <table className="w-full text-left">
                                <thead className="bg-white/5">
                                    <tr>
                                        ...
                                        <th className="p-5 text-[9px] font-black text-slate-500 uppercase tracking-widest">Winning No.</th>
                                        ...
                                        <th className="p-5 text-[9px] font-black text-slate-500 uppercase tracking-widest text-right">Dealer Prof</th>
                                        <th className="p-5 text-[9px] font-black text-slate-500 uppercase tracking-widest text-right">Platform Net</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-white/5 font-mono text-xs">
                                    {summary.games.map((g: any) => (
                                        <tr key={g.gameName} className="hover:bg-white/[0.02]">
                                            ...
                                            <td className="p-5 text-right text-amber-500">{g.totalDealerProfit.toLocaleString()}</td>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {!hasWinner ? (
                                        <button 
                                            ...
                                            className="col-span-2 py-4 rounded-xl bg-red-600 text-white font-black text-xs uppercase tracking-widest hover:bg-red-500 transition-all"
                                        >
                                            DECLARE
                                        </button>
                                    ) : (
            ...
            {activeTab === 'dealers' && (
                <div className="animate-fade-in space-y-6">
                    <div className="flex justify-between items-center">
                        <h3 className="text-xl font-black text-white russo uppercase tracking-tighter">Authorized Dealer Grid</h3>
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-white/5">
                                <tr>
                                    ...
                                    <th className="p-6 text-[10px] font-black text-slate-500 uppercase tracking-widest">Financial Parameters</th>
                                    <th className="p-6 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">Pool Liquidity</th>
                                    <th className="p-6 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">Command</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={dealer.id} className="hover:bg-white/[0.02]">
                                        <td className="p-6">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 rounded-xl bg-slate-950 border border-white/10 flex items-center justify-center font-black text-red-500 russo">{dealer.name.charAt(0)}</div>
                                                <div>
                                                    <p className="font-bold text-white uppercase tracking-widest text-sm">{dealer.name}</p>
                                                    <p className="text-[10px] text-slate-500 font-mono">NODE: {dealer.id}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="p-6">
                                            <div className="flex gap-4 text-[10px] font-black uppercase tracking-widest">
                                                <span className="text-emerald-500">Comm: {dealer.commissionRate}%</span>
                                                <span className="text-amber-500">2D: {dealer.prizeRates.twoDigit}</span>
                                            </div>
                                        </td>
                                        <td className="p-6 text-right">
                                            <p className="font-mono text-white text-base font-bold">PKR {dealer.wallet.toLocaleString()}</p>
                                        </td>
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-white/5">
                                <tr>
                                    ...
                                    <th className="p-6 text-[10px] font-black text-slate-500 uppercase tracking-widest">Overrides</th>
                                    <th className="p-6 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">Available Funds</th>
                                ...
                                {users.map((user: User) => (
                                    <tr key={user.id} className="hover:bg-white/[0.02]">
                                        <td className="p-6">
                                            ...
                                            <p className="text-[10px] text-slate-500 font-mono">PARENT: {user.dealerId} | ID: {user.id}</p>
                                        </td>
                                        <td className="p-6">
                                            <div className="flex gap-4 text-[10px] font-black uppercase tracking-widest">
                                        ...
                                        <td className="p-6 text-right">
                                            <p className="font-mono text-white text-base font-bold">PKR {user.wallet.toLocaleString()}</p>
                                        </td>
                                        <td className="p-6 text-right">
            ...
            <Modal isOpen={isDealerModalOpen} onClose={() => setIsDealerModalOpen(false)} title={selectedDealer ? "Sync Dealer Parameters" : "Authorize New Node"}>
                <DealerForm dealer={selectedDealer} onClose={() => setIsDealerModalOpen(false)} onSave={onSaveDealer} />
            </Modal>

            <Modal isOpen={isUserModalOpen} onClose={() => setIsUserModalOpen(false)} title="Override User Parameters">
                    ...
                    <table className="w-full text-left font-mono text-[10px]">
                        <thead className="bg-white/5">
                            <tr>
                                ...
                                <th className="p-4 text-slate-500 uppercase">Description</th>
