...
const Ball: React.FC<{ 
  ...
  // The winning ball is handled separately during delivery and hold to stay in front and travel the pipe
  if (isActuallyWinner && (phase === 'DELIVERY' || phase === 'HOLD' || phase === 'REVEAL')) {
      return null;
  }

  // Remaining balls stay visible and active during all phases except IDLE (where they are packed)
    ...
    const gen = async () => {
        try {
            ...
            const resp = await ai.models.generateContent({
                ...
                contents: { parts: [{ text: "High-contrast 8k ultra-realistic scientific lottery draw lab background, dark glass surfaces, amber ambient lighting, cinematic blur." }] },
    ...
    <div className="fixed inset-0 z-[10000] lottery-machine-viewport select-none bg-black overflow-hidden flex flex-col items-center justify-center font-inter">
      {aiBackdrop && (
        <div className="absolute inset-0 z-0">
          <img src={aiBackdrop} className="w-full h-full object-cover opacity-35 blur-2xl scale-110" alt="" />
        ...
        <div className="absolute top-10 text-center z-[10100] w-full px-8 animate-fade-in">
            ...
            <div className="flex flex-col items-center gap-4">
                <div className="flex items-center gap-6">
                    <div className={`w-5 h-5 rounded-full ${phase === 'SHUFFLE' ? 'bg-amber-500 animate-pulse shadow-[0_0_25px_#f59e0b]' : 'bg-emerald-500 shadow-[0_0_30px_#10b981]'}`} />
                    <p className="text-[13px] font-black text-slate-100 uppercase tracking-[0.6em]">
                        {phase === 'SHUFFLE' ? 'AERODYNAMICS: ACTIVE' : 'VALIDATION: SUCCESS'}
                    </p>
                </div>
                {phase === 'SHUFFLE' && (
                  <div className="bg-slate-900 border border-white/25 px-14 py-4 rounded-3xl shadow-3xl flex items-center gap-10 backdrop-blur-3xl">
                    <span className="text-amber-500 font-mono font-black text-3xl tracking-tighter uppercase">SYSTEM DRAW T-MINUS {timeLeft}S</span>
                  </div>
                )}
            </div>
        </div>
      ...
      {/* Main Machine Container - Stays visible throughout the reveal sequence */}
      <div className={`relative w-full h-full flex flex-col items-center justify-center transition-all duration-1000 ${phase === 'REVEAL' ? 'scale-110 brightness-50 blur-[2px]' : 'opacity-100'}`}>
        ...
        <svg style={{ position: 'absolute', width: 0, height: 0 }}>
            <defs>
                ...
                <linearGradient id="pipeGlassVolume" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stopColor="rgba(200,245,255,0.85)" />
                    <stop offset="15%" stopColor="rgba(200,245,255,0.3)" />
                    <stop offset="85%" stopColor="rgba(200,245,255,0.3)" />
                    <stop offset="100%" stopColor="rgba(200,245,255,0.85)" />
                </linearGradient>
                <linearGradient id="pipeSpecularRim" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stopColor="white" stopOpacity="1" />
                    <stop offset="3%" stopColor="white" stopOpacity="0.35" />
                    <stop offset="97%" stopColor="white" stopOpacity="0.35" />
        ...
        {/* 2. PIPELINE FRONT LAYER (Surface Glare) */}
        <div className="absolute inset-0 glass-front-highlights pipeline-container">
            <svg viewBox="0 0 400 800" preserveAspectRatio="none" className="pipeline-svg">
                ...
                <path d={pipelinePath} className="glass-specular-beam" stroke="url(#pipeCenterBeam)" strokeWidth="26" fill="none" strokeLinejoin="round" strokeLinecap="round" />
                
                {/* MECHANICAL PORTS */}
                <g opacity="1">
                    <circle cx="200" cy="400" r="54" fill="rgba(10,20,35,0.98)" stroke="white" strokeWidth="2" />
                    <circle cx="200" cy="400" r="46" fill="none" stroke="#f59e0b" strokeWidth="10" />
                    <circle cx="200" cy="400" r="38" fill="none" stroke="white" strokeWidth="1" opacity="0.6" />
                </g>
            </svg>
        </div>

        {/* 3. EXTRACTION WINNER BALL - Only this ball shows the number */}
        {(phase === 'DELIVERY' || phase === 'HOLD' || phase === 'REVEAL') && (
            ...
            >
                <div className="absolute inset-0 bg-gradient-to-tr from-white/40 via-white/10 to-transparent blur-[1.5px] rounded-full pointer-events-none" />
        ...
        {/* THE SPHERICAL MIXING JAR (PORT) - Balls stay here at all times */}
        ...
        <div className={`result-display-box transition-all duration-1000 ${phase === 'SHUFFLE' ? 'opacity-0 translate-y-48 scale-90' : 'opacity-100 translate-y-0 scale-100'}`}>
            <div className="absolute -top-16 left-1/2 -translate-x-1/2 flex flex-col items-center">
                <div className="bg-slate-950 border-2 border-amber-500 px-14 py-4 rounded-full shadow-3xl">
                    <p className="text-[15px] font-black text-amber-500 uppercase tracking-[0.9em] whitespace-nowrap">BOARD COLLECTION PORT</p>
                </div>
                <div className="w-2 h-16 bg-gradient-to-b from-amber-500 to-transparent"></div>
            </div>
            
            {/* The winning number text is hidden from the tray; only the ball shows the number */}
            {(phase === 'HOLD' || phase === 'REVEAL') ? (
                <div className="flex flex-col items-center gap-3 animate-pulse">
                    <div className="w-12 h-1.5 bg-amber-500/50 rounded-full" />
                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em]">AUTHENTICATED</p>
                </div>
            ) : (
      ...
      {/* FINAL DRAMATIC REVEAL IMPACT - Overlaying the machine */}
      {phase === 'REVEAL' && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 backdrop-blur-[4px] z-[10020] p-10">
            <div className="relative text-center space-y-16 max-w-6xl animate-result-slam-3d">
                <div className="space-y-10">
                    <div className="inline-block bg-amber-500/30 border-2 border-amber-500 px-12 py-5 rounded-full mb-4 shadow-[0_0_60px_rgba(245,158,11,0.4)]">
                        <p className="text-white font-black text-[18px] uppercase tracking-[1.8em] animate-pulse">VERIFIED OUTCOME</p>
                    </div>
                ...
                {/* Visual anchor for the result - The result number text is hidden to rely on the physical ball visibility behind the glass */}
                <div className="h-40 sm:h-64 flex items-center justify-center">
                   {/* This space is intentionally transparent to draw focus to the winner ball in the tray behind it */}
                </div>
                ...
                <div className="pt-20">
                    ...
                    >
                        <span className="relative z-10">CONFIRM VERIFICATION</span>
