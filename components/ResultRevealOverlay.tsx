...
const SHUFFLE_TIME = 45000; // 45 seconds shuffle as requested
const DELIVERY_TIME = 4500; // 4.5 seconds travel
const HOLD_TIME = 5000;    // 5 seconds hold as requested
...
const Ball: React.FC<{ 
  ...
  // Use numeric comparison for matching to avoid padding issues (e.g. "5" vs "05")
  ...
  if (isActuallyWinner && (phase === 'DELIVERY' || phase === 'HOLD')) return null;
    ...
    <div className="fixed inset-0 z-[10000] lottery-machine-viewport select-none bg-black overflow-hidden flex flex-col items-center justify-center">
      ...
      {phase !== 'REVEAL' && (
        <div className="absolute top-8 sm:top-12 text-center z-[10010] w-full px-4 animate-fade-in">
            <h2 className="text-white text-3xl sm:text-5xl font-black russo tracking-[0.2em] uppercase mb-1 drop-shadow-[0_0_20px_rgba(255,255,255,0.3)]">
            ...
            <div className="flex flex-col items-center justify-center gap-2">
                <div className="flex items-center gap-3">
                    ...
                    <p className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest">
                        {phase === 'SHUFFLE' ? 'ENTROPY STABILIZATION' : phase === 'DELIVERY' ? 'EXTRACTION IN PROGRESS' : phase === 'HOLD' ? 'VERIFICATION HOLD' : 'PROCESSING...'}
                    </p>
                </div>
                {phase === 'SHUFFLE' && (
                  <div className="bg-slate-900/80 border border-white/10 px-4 py-1 rounded-full">
                    <span className="text-amber-500 font-mono font-bold text-xs">EXTRACTION IN {timeLeft}s</span>
                  </div>
                )}
            </div>
        </div>
      ...
      <div className={`relative w-full h-full flex flex-col items-center justify-center transition-all duration-1000 ${phase === 'REVEAL' ? 'opacity-0 scale-150 blur-3xl' : 'opacity-100'}`}>
        <div className="machine-chamber">
            ...
            <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 w-24 sm:w-32 h-12 sm:h-16 bg-slate-900 rounded-t-3xl border-x-4 border-t-4 border-slate-800 z-30 flex items-center justify-center">
                <div className="w-12 sm:w-16 h-6 sm:h-8 bg-black rounded-full shadow-inner border border-white/5" />
            </div>
        </div>

        <div className="absolute z-10 pointer-events-none opacity-20 hidden sm:block">
        ...
        <div className="result-display-box">
            {phase === 'HOLD' && (
                <div className="absolute -top-12 sm:-top-16 left-1/2 -translate-x-1/2 text-emerald-400 text-[12px] sm:text-[14px] font-black uppercase tracking-[0.5em] animate-pulse whitespace-nowrap bg-black/80 px-4 py-1 rounded-full border border-emerald-500/30">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/95 z-[10020] animate-fade-in p-6 overflow-hidden">
            ...
            <div className="relative text-center space-y-8 sm:space-y-12 max-w-2xl animate-result-slam-3d">
                <div className="space-y-3 sm:space-y-4">
                    <p className="text-amber-500 font-black text-[10px] sm:text-sm uppercase tracking-[0.8em] animate-pulse drop-shadow-[0_0_10px_rgba(245,158,11,0.5)]">AUTHENTIC WINNER</p>
                    <h2 className="text-white text-4xl sm:text-7xl font-black russo tracking-tighter uppercase drop-shadow-2xl">{gameName}</h2>
                </div>
                <div className="relative inline-block px-12 py-8 sm:px-24 sm:py-20 bg-white/[0.03] rounded-[3rem] sm:rounded-[6rem] border-2 border-amber-500/40 shadow-[0_0_100px_rgba(245,158,11,0.2)] group backdrop-blur-3xl overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-br from-amber-500/10 to-transparent" />
                    <span className="relative text-[8rem] sm:text-[18rem] font-black russo text-white gold-shimmer tracking-tighter leading-none block drop-shadow-[0_15px_30px_rgba(0,0,0,0.8)]">
                        {winningNumber.padStart(2, '0')}
                    </span>
                </div>
                <div className="pt-8 sm:pt-12">
                    <button 
                        onClick={onClose}
                        className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-black px-12 py-4 sm:px-16 sm:py-5 rounded-2xl uppercase tracking-[0.4em] text-[10px] sm:text-sm transition-all transform active:scale-95 shadow-[0_0_30px_rgba(245,158,11,0.3)]"
                    >
                        CONTINUE
                    </button>
                </div>
